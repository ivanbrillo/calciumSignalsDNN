...
def create_colors(label: str, database: pd.DataFrame, ax=None) -> tuple[dict, list]:
    # unique_values = {i[label] for i in database}
    unique_values = database[label].unique()
    # print(database[label])
    # print(database[label].unique())
...
def get_colors_list(database: pd.DataFrame, colors: dict, label: str) -> list:
    return [colors[i] for i in database[label]]
